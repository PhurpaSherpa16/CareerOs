  ...
  return (
    <div className="flex items-center justify-center w-full max-w-2xl mx-auto px-4">
      {steps.map((step, index) => {
        ...
        // Only allow clicking the immediately preceding step (activeStep - 1)
        ...
        return (
          <div key={step.id} className="flex items-center flex-1 last:flex-none">
            ...
            >
              ...
              <div
                className={`size-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${
                  isActive
                    ? "bg-(--primaryBlue) text-white shadow-md shadow-(--primaryBlue)/30 ring-4 ring-(--primaryBlue)/20"
                    : isCompleted
                    ? "bg-(--primaryBlue) text-white"
                    : "bg-(--lightBlack)/20 text-(--secondaryBlack)"
            ...
            {!isLast && (
              <div
                className={`h-0.5 flex-1 mx-4 sm:mx-8 mb-6 transition-colors duration-300 ${
                  isCompleted ? "bg-(--primaryBlue)" : "bg-(--lightBlack)/30"
                }`}
              />
            )}
          </div>
